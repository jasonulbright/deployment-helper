...
Describe 'Connect-CMSite' {
    ...
    It 'Returns true on successful connection' {
        $cmBin = Join-Path $TestDrive 'AdminConsole\bin'
        New-Item -ItemType Directory -Path $cmBin -Force | Out-Null
        $env:SMS_ADMIN_UI_PATH = $cmBin
        Set-Content -Path (Join-Path $TestDrive 'AdminConsole\ConfigurationManager.psd1') -Value '@{ ModuleVersion = "1.0" }'
...
Describe 'Write-DeploymentLog' {
    It 'Creates JSONL file with correct fields' {
        ...
        $content.Trim() | Should -Match '"Timestamp":"\d{4}-\d{2}-\d{2}T'
    }
